      ...
      compiler.hooks.compilation.tap('ResourceHintWebpackPlugin', compilation => {
        if (compilation.hooks.htmlWebpackPluginAlterAssetTags) {
          compilation.hooks.htmlWebpackPluginAlterAssetTags.tapAsync('ResourceHintWebpackPluginAlterAssetTags',
            resourceHintWebpackPluginAlterAssetTags(compilation)
          );
        }
      });
    } else {
 ...
 */
const resourceHintWebpackPluginAlterAssetTags = compilation =>
...
function findAssetsByTags (filter, assetTags) {
  return assetTags
...
function findAssetsByStats (filter, compilation) {
 ...
 */
function addResourceHintTags (resourceHintType, filter, assetTags, htmlWebpackPluginOptions, compilation) {
  let urls = findAssetsByTags(filter, assetTags);

  // only search on stats if none asset was found
  if (!urls.length) {
    urls = findAssetsByStats(filter, compilation);
  }
